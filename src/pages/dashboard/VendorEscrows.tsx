...
const VendorEscrows = () => {
  ...
  // Get completable transactions from selection (in_progress jobs)
  const getCompletableSelected = () => {
    return paginatedTransactions.filter(
      tx => selectedIds.has(tx.id) && tx.status === 'in_progress'
    );
  };

  const handleBulkMarkComplete = async () => {
    const completable = getCompletableSelected();
    if (completable.length === 0) {
      toast({
        title: "No eligible jobs",
        description: "Only in-progress jobs can be marked for completion.",
        variant: "destructive"
      });
      return;
    }
    ...
    try {
      ...
      toast({
        ...
        description: `Successfully marked ${completable.length} job(s) as pending release.`
      });
          ...
          <div className="flex items-center justify-between p-4 rounded-xl bg-primary/10 border border-primary/20">
            ...
            <div className="flex items-center gap-2">
              <Button 
                ...
                disabled={bulkActionLoading || getCompletableSelected().length === 0}
              >
                {bulkActionLoading ? (
                ...
                Mark Complete ({getCompletableSelected().length})
              </Button>
            </div>
          </div>
        )}
